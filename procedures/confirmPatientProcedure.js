...
const INVOICE_TABLE = process.env.INVOICE_TABLE || `${process.env.stage || 'dev'}InvoiceTable`;
...
exports.handler = async (event) => {
  try {
    ...
    if (requirements && Array.isArray(requirements)) {
      for (const med of requirements) {
        ...
        if (availableStocks.length === 0) {
          stockWarnings.push({ medicineId, medicineType, quantity, reason: 'No stock available' });
        ...
        if (remainingQuantity > 0) {
          stockWarnings.push({ medicineId, medicineType, quantity, available: quantity - remainingQuantity, reason: 'Insufficient stock' });
        }

        // Update stocks and medicine balance only for available quantity
        ...
        // Update medicine balance quantity only for available quantity
        const totalDeducted = quantity - remainingQuantity;
        if (totalDeducted > 0) {
          await docClient.update({
    ...
    if (!patientId) {
      console.log('Warning: No patientId found, skipping invoice creation');
    } else {
      const now = formatDate(new Date().toISOString());
      const invoiceId = `INV-${Date.now()}`;

      // Check for existing unpaid invoice for this patient
      ...
      if (existingInvoices.Items && existingInvoices.Items.length > 0) {
        ...
        console.log('Updating invoice:', existingInvoice.invoiceId, 'newTotal:', updatedTotal, 'newBalance:', updatedBalance);
